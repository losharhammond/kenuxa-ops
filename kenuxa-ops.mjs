...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[KENUXA OPS] Failed to respawn launcher: ${
...
const respawnWithoutCompileCacheIfNeeded = () => {
  ...
  const env = {
    ...
    KENUXA OPS_SOURCE_COMPILE_CACHE_RESPAWNED: "1",
  };
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  const env = {
    ...
    KENUXA OPS_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:KENUXA OPS/KENUXA OPS#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g KENUXA OPS@latest`.");
  return lines.join("\n");
};
