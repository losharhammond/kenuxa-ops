...
async function main() {
  ...
  const tmpHome = mkdtempSync(path.join(os.tmpdir(), "KENUXA OPS-extension-memory-"));
  const hookPath = path.join(tmpHome, "measure-rss.mjs");
  const jsonPath = options.jsonPath ?? path.join(os.tmpdir(), "KENUXA OPS-extension-memory.json");
