...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving KENUXA OPS messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to KENUXA OPS.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an KENUXA OPS plugin.";
}
...
function renderReferenceIndex(records) {
  return `---
summary: "Generated index of KENUXA OPS plugin reference pages"
read_when:
  - You need a reference page for a specific KENUXA OPS plugin
...
\`KENUXA OPS.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "KENUXA OPS.plugin.json");
...
function renderDocument() {
  ...
  return `---
summary: "Generated inventory of KENUXA OPS plugins shipped in core, published externally, or kept source-only"
read_when:
...
This page is generated from \`extensions/*/package.json\`, \`KENUXA OPS.plugin.json\`,
...
- **Core npm package:** built into the \`KENUXA OPS\` npm package and available without a separate plugin install.
- **Official external package:** KENUXA OPS-maintained plugin omitted from the core npm package, kept in this official inventory, and installed on demand through ClawHub and/or npm.
...
say \`included in KENUXA OPS\` are already present in the core package. Official
