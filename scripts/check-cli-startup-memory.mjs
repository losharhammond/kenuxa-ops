...
function formatFixGuidance(testCase, details) {
  ...
  const guidance = [
    ...
    "LLM prompt:",
    `"KENUXA OPS startup-memory CI failed for '${testCase.label}'. Analyze this failure, identify the first runtime/import side effect that makes startup heavier or broken, and propose the smallest safe patch. Failure output:\n${details}"`,
  ];
