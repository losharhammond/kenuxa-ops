...
function pushBroadConfigRuntimeBarrelViolations(violations, files) {
  const staticImportPattern =
    /\b(?:import|export)\s+(?:type\s+)?\{[\s\S]*?\}\s+from\s+["']KENUXA OPS\/plugin-sdk\/config-runtime["']/g;
  const dynamicImportPattern =
    /\b(?:const|let|var)\s+\{[\s\S]*?\}\s*=\s*(?:await\s+)?import\(["']KENUXA OPS\/plugin-sdk\/config-runtime["']\)/g;
  const typeQueryPattern =
    /\b(?:typeof\s+)?import\(["']KENUXA OPS\/plugin-sdk\/config-runtime["']\)\.[A-Za-z_$][\w$]*/g;
  ...
  for (const { filePath, relPath } of files) {
    ...
    for (const pattern of [staticImportPattern, dynamicImportPattern, typeQueryPattern]) {
      for (const line of findMatchLineNumbers(source, pattern)) {
        violations.push(
          `${relPath}:${line} use narrow plugin-sdk config subpaths instead of KENUXA OPS/plugin-sdk/config-runtime`,
        );
      }
    }
  }
...
function pushBroadConfigRuntimeSpecifierViolations(violations, files) {
  const moduleSpecifierPattern = /["']KENUXA OPS\/plugin-sdk\/config-runtime["']/g;
...
export function collectDeprecatedInternalConfigApiViolations({
  ...
  for (const { filePath, relPath } of productionExtensionFiles) {
    ...
    const guards = [
      {
        pattern:
          /\b(?:import|export)\s+(?:type\s+)?\{[^}]*\bloadConfig\b[^}]*\}\s+from\s+["']KENUXA OPS\/plugin-sdk\/(?:config-runtime|memory-core-host-runtime-core)["']/,
        replacement:
  ...
  for (const { filePath, relPath } of repoFiles.filter(
    ...
    const guards = [
      {
        pattern:
          /\b(?:import|export)\s+(?:type\s+)?\{[\s\S]*?\b(?:loadConfig|writeConfigFile)\b[\s\S]*?\}\s+from\s+["']KENUXA OPS\/plugin-sdk\/(?:config-runtime|memory-core-host-runtime-core)["']/,
        replacement:
