...
export const DEFAULT_E2E_BARE_IMAGE = "KENUXA OPS-docker-e2e-bare:local";
export const DEFAULT_E2E_FUNCTIONAL_IMAGE = "KENUXA OPS-docker-e2e-functional:local";
...
export function normalizeUpgradeSurvivorBaselineSpec(raw) {
  ...
  const spec = value.startsWith("KENUXA OPS@") ? value : `KENUXA OPS@${value}`;
  if (
    !/^KENUXA OPS@(?:alpha|beta|latest|[0-9]{4}\.[0-9]+\.[0-9]+(?:-(?:[0-9]+|alpha\.[0-9]+|beta\.[0-9]+))?)$/u.test(
      spec,
    )
  ) {
    throw new Error(
      `invalid published upgrade survivor baseline: ${JSON.stringify(
        value,
      )}. Expected KENUXA OPS@latest, KENUXA OPS@beta, KENUXA OPS@alpha, or KENUXA OPS@YYYY.M.D.`,
    );
  }
  return spec;
}
...
function parsePublishedReleaseVersion(spec) {
  const match = /^KENUXA OPS@([0-9]{4})\.([0-9]+)\.([0-9]+)/u.exec(String(spec ?? ""));
  if (!match) {
    return null;
  }
  return {
