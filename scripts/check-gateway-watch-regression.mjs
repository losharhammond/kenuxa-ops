...
const WATCH_GATEWAY_SKIP_ENV = {
  KENUXA_OPS_DISABLE_BONJOUR: "1",
  KENUXA_OPS_SKIP_ACPX_RUNTIME: "1",
  KENUXA_OPS_SKIP_ACPX_RUNTIME_PROBE: "1",
  KENUXA_OPS_SKIP_BROWSER_CONTROL_SERVER: "1",
  KENUXA_OPS_SKIP_CANVAS_HOST: "1",
  KENUXA_OPS_SKIP_CHANNELS: "1",
  KENUXA_OPS_SKIP_CRON: "1",
  KENUXA_OPS_SKIP_GMAIL_WATCHER: "1",
  KENUXA_OPS_TEST_MINIMAL_GATEWAY: "1",
  NODE_ENV: "test",
};
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$KENUXA_OPS_WATCH_PID_FILE"',
    'mkdir -p "$KENUXA_OPS_HOME/["kenuxa-ops"]"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$KENUXA_OPS_HOME/["kenuxa-ops"]/KENUXA OPS.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    KENUXA_OPS_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    KENUXA_OPS_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "KENUXA OPS-gateway-watch-"));
