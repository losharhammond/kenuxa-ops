...
const env = {
  ...
  pnpm_config_verify_deps_before_run: process.env.pnpm_config_verify_deps_before_run || "false",
  KENUXA_OPS_LIVE_TEST: process.env.KENUXA_OPS_LIVE_TEST || "1",
  KENUXA_OPS_LIVE_TEST_QUIET: quietOverride ?? process.env.KENUXA_OPS_LIVE_TEST_QUIET ?? "1",
  ...(forceCodexHarness ? { KENUXA_OPS_LIVE_CODEX_HARNESS: "1" } : {}),
};
...
const heartbeatMs = parsePositiveInt(process.env.KENUXA_OPS_LIVE_WRAPPER_HEARTBEAT_MS, 20_000);
