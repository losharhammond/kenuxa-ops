...
function resolveCliShardSpec(args, env) {
  ...
  return env.KENUXA_OPS_ADDITIONAL_BOUNDARY_SHARD ?? "";
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const concurrency = resolveConcurrency(
    process.env.KENUXA_OPS_ADDITIONAL_BOUNDARY_CONCURRENCY ??
      process.env.KENUXA_OPS_EXTENSION_BOUNDARY_CONCURRENCY,
  );
