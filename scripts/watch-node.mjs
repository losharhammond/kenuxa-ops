...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.KENUXA OPS_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[KENUXA OPS] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  console.error("");
  console.error(
    "[KENUXA OPS] gateway:watch could not start because a dependency package config looks corrupted.",
  );
  if (packageConfigPath) {
    console.error(`[KENUXA OPS] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[KENUXA OPS] This usually means a file in node_modules is empty or truncated.");
  console.error("[KENUXA OPS] Recommended recovery:");
  console.error("[KENUXA OPS]   rm -rf node_modules");
  console.error("[KENUXA OPS]   pnpm store prune");
  console.error("[KENUXA OPS]   pnpm install");
  console.error("");
  console.error("[KENUXA OPS] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.KENUXA OPS_WATCH_MODE = "1";
  childEnv.KENUXA OPS_WATCH_SESSION = watchSession;
  ...
  childEnv.KENUXA OPS_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.KENUXA OPS_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `KENUXA OPS doctor --fix --non-interactive` once.",
        deps,
      );
