...
const DEFAULT_GITHUB_WORKFLOW = "KENUXA OPS-live-and-e2e-checks-reusable.yml";
...
function dockerPreflightContainerNames(raw) {
  return raw
    ...
    .filter((name) =>
      /^(?:KENUXA OPS-(?:gateway-e2e|openwebui|openwebui-gateway|config-reload-e2e)-)/.test(name),
    );
}
...
async function prepareKENUXA OPSPackage(baseEnv, logDir) {
  ...
  const packDir = path.join(logDir, "KENUXA OPS-package");
  await mkdir(packDir, { recursive: true });
  const packageTgz = path.join(packDir, "KENUXA OPS-current.tgz");
  await runForeground(
    "Prepare KENUXA OPS package once",
    `node scripts/package-KENUXA OPS-for-docker.mjs --output-dir ${shellQuote(packDir)} --output-name KENUXA OPS-current.tgz`,
    baseEnv,
  );
