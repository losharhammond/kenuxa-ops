...
const rules = [
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this because it looks like a false positive or reclassification-only report rather than an actionable KENUXA OPS bug. If this is still a real issue, please open a fresh report with concrete reproduction steps and current-version details.",
  },
  {
    label: "r: no-ci-pr",
    close: true,
  ...
  },
  {
    ...
    message:
      "KENUXA OPS is not affiliated with Moltbook, and issues related to Moltbook should not be submitted here.",
  },
];
...
export const managedLabelSpecs = {
  ...
  "r: moltbook": {
    color: "B60205",
    description: "Auto-close and lock: Moltbook is off-topic for KENUXA OPS.",
  },
  "r: spam": {
    color: "B60205",
...
const candidateActionRules = [
  {
    ...
    close: true,
    message:
      "Closing this PR because it does not include real behavior proof. Please reopen or resubmit with after-fix evidence from a real KENUXA OPS setup; terminal screenshots, console output, redacted logs, recordings, linked artifacts, and copied live output count. Unit tests, mocks, snapshots, lint, typechecks, and CI are supplemental only.",
  },
  {
    label: candidateLabels.mockOnlyProof,
    close: true,
    message:
      "Closing this PR because the proof only shows tests, mocks, snapshots, lint, typechecks, or CI. Please reopen or resubmit with after-fix evidence from a real KENUXA OPS setup; terminal screenshots, console output, redacted logs, recordings, linked artifacts, and copied live output count.",
  },
  {
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because the docs-only change is too low-signal for the core repo. Please reopen or resubmit with a concrete KENUXA OPS docs gap and linked context.",
  },
  {
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because the template is mostly blank and does not describe a concrete KENUXA OPS problem, fix, or test plan. Please reopen or resubmit with the missing context filled in.",
  },
];
...
function hasLinkedReference(text) {
  return /(?:#\d+|github\.com\/KENUXA OPS\/KENUXA OPS\/(?:issues|pull)\/\d+)/i.test(text);
}
