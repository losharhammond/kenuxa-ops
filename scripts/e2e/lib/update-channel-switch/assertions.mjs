...
function assertConfigChannel(channel) {
  const config = readJson(path.join(process.env.HOME, ".KENUXA OPS", "KENUXA OPS.json"));
  if (config.update?.channel === channel) {
    return;
  }
  if (process.env.KENUXA OPS_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1") {
    console.log(
...
switch (command) {
  case "prepare-git-fixture":
    prepareGitFixture(args[0] ?? "/tmp/KENUXA OPS-git");
    break;
  case "write-control-ui":
    writeControlUi(args[0] ?? "/tmp/KENUXA OPS-git");
    break;
  case "assert-update":
    assertUpdate(args[0]);
    break;
