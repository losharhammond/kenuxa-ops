...
const CHUNKED_PAYLOAD_MARKER = "__KENUXA OPSQaCredentialPayloadChunksV1";
...
function normalizeCredentialRole() {
  const raw =
    process.env.KENUXA OPS_NPM_TELEGRAM_CREDENTIAL_ROLE?.trim() ||
    process.env.KENUXA OPS_QA_CREDENTIAL_ROLE?.trim() ||
...
function normalizeEndpointPrefix() {
  const raw = process.env.KENUXA OPS_QA_CONVEX_ENDPOINT_PREFIX?.trim() || DEFAULT_ENDPOINT_PREFIX;
  if (!raw.startsWith("/") || raw.startsWith("//") || raw.includes("\\") || raw.includes("..")) {
    throw new Error(
      "KENUXA OPS_QA_CONVEX_ENDPOINT_PREFIX must be an absolute path like /qa-credentials/v1.",
    );
  }
...
function resolveConfig() {
  const siteUrl = process.env.KENUXA OPS_QA_CONVEX_SITE_URL?.trim();
  if (!siteUrl) {
    throw new Error("Missing KENUXA OPS_QA_CONVEX_SITE_URL for --credential-source convex.");
  }
  const parsed = new URL(siteUrl);
  const allowInsecure = /^(1|true|yes)$/iu.test(process.env.KENUXA OPS_QA_ALLOW_INSECURE_HTTP ?? "");
  const isLoopback = parsed.hostname === "localhost" || parsed.hostname === "127.0.0.1";
  if (
    parsed.protocol !== "https:" &&
    !(parsed.protocol === "http:" && allowInsecure && isLoopback)
  ) {
    throw new Error("KENUXA OPS_QA_CONVEX_SITE_URL must use https://.");
  }
  const role = normalizeCredentialRole();
  const authToken =
    role === "ci"
      ? process.env.KENUXA OPS_QA_CONVEX_SECRET_CI?.trim()
      : process.env.KENUXA OPS_QA_CONVEX_SECRET_MAINTAINER?.trim();
  if (!authToken) {
    throw new Error(
      role === "ci"
        ? "Missing KENUXA OPS_QA_CONVEX_SECRET_CI for CI credential access."
        : "Missing KENUXA OPS_QA_CONVEX_SECRET_MAINTAINER for maintainer credential access.",
    );
  }
  const endpointPrefix = normalizeEndpointPrefix();
  const ownerId =
    process.env.KENUXA OPS_QA_CREDENTIAL_OWNER_ID?.trim() ||
  ...
  return {
    ...
    acquireTimeoutMs: parsePositiveInteger(
      process.env.KENUXA OPS_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS,
      DEFAULT_ACQUIRE_TIMEOUT_MS,
      "KENUXA OPS_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS",
    ),
    heartbeatIntervalMs: parsePositiveInteger(
      process.env.KENUXA OPS_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS,
      DEFAULT_HEARTBEAT_INTERVAL_MS,
      "KENUXA OPS_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS",
    ),
    heartbeatUrl: joinEndpoint("heartbeat"),
    httpTimeoutMs: parsePositiveInteger(
      process.env.KENUXA OPS_QA_CREDENTIAL_HTTP_TIMEOUT_MS,
      DEFAULT_HTTP_TIMEOUT_MS,
      "KENUXA OPS_QA_CREDENTIAL_HTTP_TIMEOUT_MS",
    ),
    leaseTtlMs: parsePositiveInteger(
      process.env.KENUXA OPS_QA_CREDENTIAL_LEASE_TTL_MS,
      DEFAULT_LEASE_TTL_MS,
      "KENUXA OPS_QA_CREDENTIAL_LEASE_TTL_MS",
    ),
    ownerId,
...
async function writeCredentialEnv(pathname, payload) {
  await fs.writeFile(
    pathname,
    [
      `export KENUXA OPS_QA_TELEGRAM_GROUP_ID=${shellQuote(payload.groupId)}`,
      `export KENUXA OPS_QA_TELEGRAM_DRIVER_BOT_TOKEN=${shellQuote(payload.driverToken)}`,
      `export KENUXA OPS_QA_TELEGRAM_SUT_BOT_TOKEN=${shellQuote(payload.sutToken)}`,
      "",
    ].join("\n"),
    { mode: 0o600 },
  );
