...
export const ko: TranslationMap = {
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "모바일에서 사용 중이신가요? 데스크톱에서 KENUXA OPS dashboard --no-open으로 전체 URL(#token=... 포함)을 복사하세요.",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — KENUXA OPS models auth로 다시 인증하세요",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "KENUXA OPS dashboard --no-open의 토큰을 붙여넣거나 구성된 비밀번호를 입력하세요.",
        stepGenerate:
          "토큰이 구성되어 있지 않으면 Gateway 호스트에서 KENUXA OPS doctor --generate-gateway-token을 실행하세요.",
      ...
      authFailed: {
        ...
        stepDashboard:
          "KENUXA OPS dashboard --no-open을 실행하고 새 URL을 열거나 해당 토큰을 붙여넣으세요.",
        stepReplace:
      ...
      pairing: {
        ...
        stepList: "Gateway 호스트에서 KENUXA OPS devices list를 실행하세요.",
        stepApproveId: "이 요청을 승인하세요: KENUXA OPS devices approve {requestId}.",
      ...
      protocol: {
        ...
        stepDashboard:
          "UI와 Gateway가 같은 설치에서 오도록 KENUXA OPS dashboard로 제공된 dashboard를 다시 여세요.",
        stepDevUi:
          "pnpm ui:dev를 사용하는 경우 현재 checkout 기준으로 개발 UI를 다시 빌드하거나 다시 시작하세요.",
        stepRestart:
          "KENUXA OPS를 업데이트한 뒤 Gateway를 다시 시작하여 현재 프로토콜을 제공하게 하세요.",
      },
      network: {
        title: "연결할 수 없음",
        summary:
        ...
        stepGateway:
          "KENUXA OPS status 또는 KENUXA OPS gateway run으로 Gateway가 실행 중인지 확인하세요.",
        stepUrl:
          "WebSocket URL을 확인하고 Gateway가 HTTPS/Tailscale Serve 뒤에 있으면 wss://를 사용하세요.",
        stepDashboard:
          "KENUXA OPS dashboard --no-open으로 dashboard를 다시 열어 현재 URL과 인증 세부 정보를 다시 복사하세요.",
      },
    },
  },
  chat: {
