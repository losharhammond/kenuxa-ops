...
export const zh_CN: TranslationMap = {
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "在手机上？从桌面运行 KENUXA OPS dashboard --no-open 复制完整 URL（包括 #token=...）。",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — 使用 KENUXA OPS models auth 重新认证",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "粘贴 KENUXA OPS dashboard --no-open 提供的令牌，或输入已配置的密码。",
        stepGenerate:
          "如果未配置令牌，请在 Gateway 主机上运行 KENUXA OPS doctor --generate-gateway-token。",
        stepConnect: "更新凭据后再次点击 Connect。",
      },
      authFailed: {
        title: "认证不匹配",
        summary: "提供的凭据被拒绝。最常见原因是令牌已过期，或令牌来自另一个 Gateway URL。",
        stepDashboard: "运行 KENUXA OPS dashboard --no-open 并打开新的 URL，或粘贴其中的令牌。",
      ...
      pairing: {
        ...
        stepList: "在 Gateway 主机上运行 KENUXA OPS devices list。",
        stepApproveId: "批准此请求：KENUXA OPS devices approve {requestId}。",
      ...
      protocol: {
        ...
        stepDashboard:
          "使用 KENUXA OPS dashboard 重新打开提供的 dashboard，确保 UI 和 Gateway 来自同一安装。",
        stepDevUi: "如果使用 pnpm ui:dev，请基于当前 checkout 重新构建或重启开发 UI。",
        stepRestart: "更新 KENUXA OPS 后重启 Gateway，使其提供当前协议。",
      },
      network: {
        title: "无法连接",
        summary: "浏览器无法完成 Gateway 连接。重试凭据前请检查目标和传输方式。",
        stepGateway: "使用 KENUXA OPS status 或 KENUXA OPS gateway run 确认 Gateway 正在运行。",
        stepUrl: "检查 WebSocket URL；当 Gateway 位于 HTTPS/Tailscale Serve 后面时使用 wss://。",
        stepDashboard:
          "使用 KENUXA OPS dashboard --no-open 重新打开 dashboard，以重新复制当前 URL 和认证详情。",
      },
    },
  },
  chat: {
