...
export const ja_JP: TranslationMap = {
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "モバイルを使用していますか？ デスクトップで KENUXA OPS dashboard --no-open から完全な URL（#token=... を含む）をコピーしてください。",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — KENUXA OPS models auth で再認証してください",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste:
          "KENUXA OPS dashboard --no-open のトークンを貼り付けるか、構成済みのパスワードを入力します。",
        stepGenerate:
          "トークンが構成されていない場合は、Gateway ホストで KENUXA OPS doctor --generate-gateway-token を実行します。",
      ...
      authFailed: {
        ...
        stepDashboard:
          "KENUXA OPS dashboard --no-open を実行し、新しい URL を開くか、そのトークンを貼り付けます。",
        stepReplace:
      ...
      pairing: {
        ...
        stepList: "Gateway ホストで KENUXA OPS devices list を実行します。",
        stepApproveId: "このリクエストを承認します: KENUXA OPS devices approve {requestId}.",
      ...
      protocol: {
        ...
        stepDashboard:
          "KENUXA OPS dashboard で提供元の dashboard を開き直し、UI と Gateway が同じインストールから来るようにします。",
        stepDevUi:
          "pnpm ui:dev を使用している場合は、現在の checkout に対して開発 UI を再ビルドまたは再起動します。",
        stepRestart: "KENUXA OPS 更新後に Gateway を再起動し、現在のプロトコルを提供させます。",
      },
      network: {
        title: "接続できません",
        summary:
        ...
        stepGateway:
          "KENUXA OPS status または KENUXA OPS gateway run で Gateway が実行中であることを確認します。",
        stepUrl:
          "WebSocket URL を確認し、Gateway が HTTPS/Tailscale Serve の背後にある場合は wss:// を使用します。",
        stepDashboard:
          "KENUXA OPS dashboard --no-open で dashboard を開き直し、現在の URL と認証詳細を再コピーします。",
      },
    },
  },
  chat: {
