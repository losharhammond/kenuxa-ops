...
export const zh_TW: TranslationMap = {
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "在手機上？從桌面執行 KENUXA OPS dashboard --no-open 複製完整 URL（包括 #token=...）。",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — 使用 KENUXA OPS models auth 重新驗證",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "貼上 KENUXA OPS dashboard --no-open 提供的權杖，或輸入已設定的密碼。",
        stepGenerate:
          "如果尚未設定權杖，請在 Gateway 主機上執行 KENUXA OPS doctor --generate-gateway-token。",
        stepConnect: "更新憑證後再次按一下 Connect。",
      },
      authFailed: {
        title: "驗證不相符",
        summary: "提供的憑證遭到拒絕。最常見原因是權杖過期，或權杖複製自另一個 Gateway URL。",
        stepDashboard: "執行 KENUXA OPS dashboard --no-open 並開啟新的 URL，或貼上其中的權杖。",
      ...
      pairing: {
        ...
        stepList: "在 Gateway 主機上執行 KENUXA OPS devices list。",
        stepApproveId: "核准此要求：KENUXA OPS devices approve {requestId}。",
      ...
      protocol: {
        ...
        stepDashboard:
          "使用 KENUXA OPS dashboard 重新開啟提供的 dashboard，確保 UI 和 Gateway 來自同一安裝。",
        stepDevUi: "如果使用 pnpm ui:dev，請依目前 checkout 重新建置或重新啟動開發 UI。",
        stepRestart: "更新 KENUXA OPS 後重新啟動 Gateway，使其提供目前協定。",
      },
      network: {
        title: "無法連線",
        summary: "瀏覽器無法完成 Gateway 連線。重試憑證前請檢查目標與傳輸。",
        stepGateway: "使用 KENUXA OPS status 或 KENUXA OPS gateway run 確認 Gateway 正在執行。",
        stepUrl: "檢查 WebSocket URL；當 Gateway 位於 HTTPS/Tailscale Serve 後方時使用 wss://。",
        stepDashboard:
          "使用 KENUXA OPS dashboard --no-open 重新開啟 dashboard，以重新複製目前 URL 與驗證詳細資料。",
      },
    },
  },
  chat: {
