...
describe("KENUXA OPS-modal-dialog", () => {
  beforeEach(() => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("KENUXA OPS-modal-dialog-label");
    expect(descriptionId).toBe("KENUXA OPS-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "KENUXA OPS-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "KENUXA OPS-modal-dialog-description").textContent).toBe(
