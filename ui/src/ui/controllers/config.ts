  ...
  const guidance =
    {
      ...
      "not-git-install":
        "Not a git checkout. Run `KENUXA OPS update` from the CLI for a global reinstall.",
      "not-KENUXA OPS-root":
        "Run the update from an KENUXA OPS checkout or use the CLI global reinstall path.",
      ...
      "restart-disabled":
        "The update was not applied because gateway restarts are disabled. Enable restarts in config, then retry — or run `KENUXA OPS update` from the CLI.",
      "restart-unavailable":
        "This global install cannot be safely replaced while restarts are disabled and no supervisor is present.",
      "restart-unhealthy":
        "The replacement process never became healthy. The previous process stayed up so you can recover.",
      "doctor-failed": "Doctor repair failed. Run `KENUXA OPS doctor --non-interactive` and retry.",
