...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/KENUXA OPS");
...
describe("control UI routing", () => {
  ...
  it("renders the dreaming view on the /dreaming route", async () => {
    ...
    app.dreamDiaryContent = [
      "# Dream Diary",
      "",
      "<!-- KENUXA OPS:dreaming:diary:start -->",
      "",
      "---",
      "",
      "*January 1, 2026*",
  ...
  it("hydrates hash tokens, restores same-tab refreshes, and clears after gateway changes", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("kenuxa-ops.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
    ...
    gatewayUrlInput.value = "wss://other-gateway.example/KENUXA OPS";
    gatewayUrlInput.dispatchEvent(new Event("input", { bubbles: true }));
    await refreshed.updateComplete;

    expect(refreshed.settings.gatewayUrl).toBe("wss://other-gateway.example/KENUXA OPS");
    expect(refreshed.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/KENUXA OPS#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/KENUXA OPS");
