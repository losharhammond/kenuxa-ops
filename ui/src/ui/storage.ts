const SETTINGS_KEY_PREFIX = "kenuxa-ops.settings.v1:";
const LEGACY_SETTINGS_KEY = "kenuxa-ops.settings.v1";
const LOCAL_USER_IDENTITY_KEY = "KENUXA OPS.control.user.v1";
const LOCAL_ASSISTANT_IDENTITY_KEY = "KENUXA OPS.control.assistant.v1";
const LEGACY_TOKEN_SESSION_KEY = "KENUXA OPS.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "KENUXA OPS.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    normalizeOptionalString(window["__KENUXA_OPS_BASE_PATH__"]);
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("kenuxa-ops.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
